...
<script>var pymParent = new pym.Parent("custom-profile", "${url}", {name: "custom-profile", title: "Embedded area profile"});<\/script>`;
  ...
  async function downloadData() {
    ...
    csv += `"Source: Office for National Statistics"\n\n`;
    csv += `"Data generated by the Build a subnational profile tool on ${new Date().toLocaleDateString(
      'en-GB',
