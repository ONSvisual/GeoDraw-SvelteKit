...
<script>var pymParent = new pym.Parent("profile", "${url}", {name: "profile"});<\/script>`;
  ...
  async function downloadData() {
    ...
    csv += `"Variable","Category","${state.name}","England and Wales","Unit","Base population"\n`;
    ...
    download(file, state.name.replaceAll(' ', '_') + '.csv');
  }
</script>
    ...
    <div class="save-buttons">
      ...
      <button
        ...
        on:click={async () => {
          ...
          download(blob, `${state.name.replaceAll(' ', '_')}.json`);
        }}
      >
