<script>
  ...
  function transformData(data) {
    ...
    for (let key in indexed) {
      xDomain.forEach(x => {
        indexed[key][`${x}_change`] = (indexed[key][x] / indexed[key][xDomain[0]]);
      });
  ...
  function update(data) {
    ...
    zDomain = _data.map(d => d[zKey]).filter((v, i, a) => a.indexOf(v) === i);
...
<div class="baseline" style:top="{yScale(1)}%"/>
  <svg viewBox="0 0 100 100" preserveAspectRatio="none">
      ...
      <path d={makePath(d, )} vector-effect="non-scaling-stroke" stroke="{i == _data.length - 1 ? '#27A0CC' : 'black'}" stroke-width="{i == _data.length - 1 ? lineWidth + 1.5 : lineWidth}"/>
      {/each}
  </svg>
<div class="point" style:left="{xScale(xVal)}%" style:top="{yScale(_data[0][yKey(xVal)])}%"/>
{#if xVal != xDomain[0]}
<div class="point-text bold" style:left="{xScale(xVal)}%" style:top="{yScale(_data[0][yKey(xVal)])}%">{formatTick(_data[0][yKey(xVal)])}%</div>
{/if}
</div>
